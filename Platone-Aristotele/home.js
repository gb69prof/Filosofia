...
const info={approfondisco:['Percorso 03 · Ricerca','Approfondisco','Fonti antiche, passi originali, interpretazioni concorrenti e questioni ancora aperte.'],fumetti:['Percorso 04 · Narrazione','A fumetti','Le vite e le idee dei due filosofi diventeranno una narrazione storica illustrata.'],dizionario:['Strumento','Dizionario filosofico','Definizioni semplici e approfondite, termini greci e rimandi.'],atlante:['Strumento','Atlante delle idee','Una rete navigabile tra concetti e problemi.'],confronti:['Strumento','Confronti continui','Le risposte dei due filosofi allo stesso problema.'],laboratorio:['Strumento','Laboratorio dei problemi','Casi, scelte, dibattiti e costruzione di argomenti.'],tempo:['Strumento','Linea del tempo','Biografie, opere e storia politica.'],biblioteca:['Strumento','Biblioteca delle opere','Dialoghi e trattati organizzati per opera e problema.'],accademia:['Esperienza','Visita l’Accademia','Una passeggiata narrativa nei luoghi della filosofia.']};
document.addEventListener('click',event=>{const key=event.target.closest('[data-section]')?.dataset.section;if(!key)return;drawer.classList.remove('open');const item=info[key];modalEye.textContent=item[0];modalTitle.textContent=item[1];modalText.textContent=item[2];modal.showModal()});
