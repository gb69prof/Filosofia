...
const info={accademia:['Esperienza','Visita l’Accademia','Una passeggiata narrativa nei luoghi della filosofia.']};
document.addEventListener('click',event=>{const key=event.target.closest('[data-section]')?.dataset.section;if(!key)return;drawer.classList.remove('open');if(key==='approfondisco'){location.href='./approfondisco/';return}if(key==='fumetti'){location.href='./fumetti/';return}if(key==='dizionario'){location.href='./dizionario/';return}if(key==='atlante'){location.href='./atlante/';return}if(key==='confronti'){location.href='./confronti/';return}if(key==='laboratorio'){location.href='./laboratorio/';return}if(key==='tempo'){location.href='./linea-del-tempo/';return}if(key==='biblioteca'){location.href='./biblioteca/';return}const item=info[key];if(!item)return;modalEye.textContent=item[0];modalTitle.textContent=item[1];modalText.textContent=item[2];modal.showModal()});
